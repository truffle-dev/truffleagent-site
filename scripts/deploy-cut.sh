#!/usr/bin/env bash
# scripts/deploy-cut.sh
# Single-entry safe-deploy wrapper for the Cut surface of truffleagent.com.
# Mirrors deploy-take.sh: sanity, bridge health, drift warn, migration
# lockstep, build, deploy, propagation wait, smoke, done.
#
# Usage:
#   bash scripts/deploy-cut.sh                      # normal deploy
#   bash scripts/deploy-cut.sh --skip-smoke         # build + deploy only
#   bash scripts/deploy-cut.sh --dry-run            # build only
#   CUT_GOLDEN_PIECE=cu_xxx bash scripts/deploy-cut.sh   # piece-bound smoke
...
step "2/9  Bridge health (cut routes live on >= 1.3.0)"
bridge_health=$(curl -sf --max-time 10 https://bridge.truffleagent.com/health 2>/dev/null || echo "")
if [[ -z "$bridge_health" ]]; then
  die "bridge unreachable at https://bridge.truffleagent.com/health — Cut pipeline would 502. Fix the bridge (supervise.sh) before deploying."
fi
...
case "$bridge_version" in
  1.[0-2].*) die "bridge $bridge_version predates the Cut routes (need >= 1.3.0)" ;;
esac
...
if [[ -n "$dirty" ]]; then
  echo "  dirty files:"
  echo "$dirty" | sed 's/^/    /'
  unrelated=$(echo "$dirty" | awk '{print $2}' | grep -v -E '(functions/(api/cut|cut|v-cut|i-cut)|functions/_cut-shared|migrations/000[0-9]_cut|scripts/(smoke|deploy)-cut|src/pages/cut)' || true)
  if [[ -n "$unrelated" ]]; then
    echo ""
    echo "  WARN: non-cut files dirty. Stash before deploy or proceed intentionally:"
...
if [[ "$SKIP_SMOKE" == "1" ]]; then
  ...
  step "8/9  Smoke"
  if bash "$REPO_ROOT/scripts/smoke-cut.sh" https://truffleagent.com "${CUT_GOLDEN_PIECE:-}"; then
    echo ""
    echo "  smoke green"
  else
    echo ""
