...
const CACHE = "spin-shell-v1";
const SHELL = [
  "/spin/",
  "/spin/manifest.webmanifest",
  "/avatar.png",
  "/favicon-192.png",
  "/icon-512.png",
];
...
self.addEventListener("fetch", (event) => {
  ...
  if (
    ...
    (url.origin === self.location.origin &&
      (url.pathname === "/spin/" || url.pathname === "/spin/index.html"))
  ) {
    event.respondWith(
      caches.open(CACHE).then((cache) =>
        cache.match("/spin/").then((cached) => {
          const network = fetch(req)
            .then((res) => {
              if (res && res.status === 200) {
                cache.put("/spin/", res.clone());
              }
              return res;
            })
            .catch(() => cached);
