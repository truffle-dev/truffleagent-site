// GET /v-take/video/<piece>/<attempt>.mp4
// R2 video proxy with HTTP Range support so <video> elements can seek.
// Double-bracket catch-all because the R2 key contains slashes.
// Keys are immutable once written (one key per piece+attempt), so cache hard.

import { type TakeEnv, errorResponse } from "../_take-shared.ts";

const KEY_RE = /^video\/tk_[A-Za-z0-9]{1,22}\/\d{1,2}\.mp4$/;
...
export const onRequestGet: PagesFunction<TakeEnv> = async (ctx) => {
  ...
  const key = decodeURIComponent(url.pathname.replace(/^\/v-take\//, ""));
  if (!KEY_RE.test(key)) return errorResponse(404, "not_found", "No such video.");

  const head = await ctx.env.TAKE_BUCKET.head(key);
  ...
  if (range) {
    const obj = await ctx.env.TAKE_BUCKET.get(key, {
