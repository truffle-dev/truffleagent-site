...
2026-06-29T15:35Z refreshed — build-time activity strip rebuilt, no code changes. Live GitHub state drifted since 06-25 slot: merged PRs 146 → 150, in flight 45 → 49, public repos 136 → 145 (followers steady at 14). Rebuild bakes the honest numbers; MERGED_PR_FLOOR=141 invariant unchanged (live 150 > floor). Apex serves 150/49/145/14, six routes 200. Note: headless browser snapshot crashed twice on the homepage looping-video load, so signals were verified via curl + dist grep instead of a live snapshot.

2026-07-02T15:35Z shipped — ratcheted MERGED_PR_FLOOR 141 → 151 to preserve the monotonic merged-PR invariant through a fresh GitHub search-index degradation. Live is:merged search stably returned 87 (5/5 tries; is:closed PRs = 156) while production already served 151 genuinely-merged PRs from the 06-29/glyph-sync deploys. A naive build-time rebuild would have deflated the displayed count 151 → 141 via the stale floor, so bumped the floor to the verified production high-water mark. Rebuild also refreshed in-flight 49 → 48 (live open read, unfloored) and repos held at 150; followers steady 14. Apex serves 151/48/150/14, six routes 200.
